...
def SOD_Metrics(mask, pred):
    # gt_threshold = 0.5
    # mask = (mask >= gt_threshold).astype(np.float32)
    ...
    MAE_.step(pred=pred, gt=mask)

    fm = FM.get_results()["fm"]
    wfm = WFM.get_results()["wfm"]
    # wfm = weighted_fmeasure(mask, pred)
    ...
    # S_measure, Wgt_F, E_measure, MAE
    return sm, wfm, em["curve"].mean(), mae
